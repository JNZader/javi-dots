...
interface Props {
  onConfirm: (kiteguard: boolean) => void
}

export default function KiteguardToggle({ onConfirm }: Props) {
  ...
  return (
    <Box flexDirection="column">
      <Text bold>Enable runtime security?</Text>

      <Box marginTop={1} marginLeft={2}>
        <Text color={enabled ? theme.success : theme.muted}>
          {enabled ? glyph.filledDot : glyph.emptyDot}{' '}
        </Text>
        <Text bold color={enabled ? theme.success : theme.muted}>kiteguard</Text>
        <Text color={theme.muted}>{'  '}</Text>
        <Text color={theme.muted}>Runtime security for Claude Code sessions</Text>
      </Box>
